...
namespace Exercise24_ExerciseTasks
{
    /// <summary>
    ...
    /// 
    /// 2 - Show the tasks between two certain dates(day, month and year). 
    /// If the user presses Enter without specifying date, it will be taken as "today". 
    /// It must display the number of each record, the date(DD/ MM/YYYY), description, 
    /// category and importance, all in the same line, separated with hyphens.
    /// 
    /// 3 - Find tasks that contain a certain text(in description or category, not case sensitive). 
    /// It will display number, date and description(only 50 letters, in case it was be longer). 
    /// The user should be notified if none is found.
    /// 
    /// 4 - Update a record(it will ask for the number, will display the previous value of each field and 
    /// the user can press Enter not to modify any of the data). The user should be warned(but not asked again) 
    /// if he enters an incorrect record number. It is not necessary to validate any of the fields.
    /// 
    /// 5 - Delete some data, between two positions indicated by the user.The user should be warned 
    /// (but not asked again) if he enters any incorrect record number. 
    /// Each record to be deleted must be displayed, and the user must be asked for confirmation.
    /// 
                    ...
                    case '4': //Update a record
                        if (counter >= 1)
                        {
                            ...
                            if ((update >= 0) && (update < counter))
                            {
                                ...
                                newValue = Console.ReadLine();
                                if (newValue != "")
                                    tasks[update].date.day = Convert.ToByte(newValue);
                            }
                            else
